...
class PipelineConfig:
	...
	# Optional per-split quotas for positive/negative records
...
def validate_config(config: PipelineConfig) -> PipelineConfig:
	...
	if cache_format not in {"npz", "tfrecord"}:
		raise ValueError("'dataset_cache_format' debe ser 'npz' o 'tfrecord'.")
...
def config_summary_df(config: PipelineConfig) -> pd.DataFrame:
	rows = []
	for field_def in fields(config):
		value = getattr(config, field_def.name)
		rows.append({"key": field_def.name, "value": value})
...
def save_cv_outputs(
	*,
	...
	cfg_path = output_dir / "config_used.json"
	cfg_path.write_text(json.dumps(config_to_dict(config), indent=2, sort_keys=True), encoding="utf-8")
	metrics_df = pd.DataFrame([asdict(fr) for fr in fold_results])
	metrics_df.to_csv(output_dir / "fold_metrics.csv", index=False)
	if predictions_df is not None:
		predictions_df.to_csv(output_dir / "fold_predictions.csv", index=False)
...
def resolve_preprocess_settings(config: PipelineConfig) -> dict[str, object]:
	base = PREPROCESS_CONFIGS.get(config.condition, PREPROCESS_CONFIGS["filtered"]).copy()
	...
	if config.preprocess_n_harmonics is not None:
		...
		base["notch_harmonics"] = config.preprocess_n_harmonics > 0
	return base
...
def resolve_checkpoint_dir(config: PipelineConfig, run_id: str) -> Path:
	if config.checkpoint_dir is not None:
		...
		base = Path(config.output_dir) / "checkpoints"
	else:
		base = Path.cwd() / "runs" / run_id / "checkpoints"
...
def resolve_epoch_time_log_path(config: PipelineConfig, run_id: str) -> Path:
	if config.epoch_time_log_path is not None:
		base_path = Path(config.epoch_time_log_path)
		base_str = str(base_path)
		if "{run_id}" in base_str:
			candidate = Path(base_str.format(run_id=run_id))
		elif base_path.suffix:
			candidate = base_path.with_name(f"{base_path.stem}_{run_id}{base_path.suffix}")
		else:
			candidate = base_path / f"{run_id}_epoch_times.csv"
		if not candidate.is_absolute():
			base_dir = Path(config.output_dir) if config.output_dir is not None else Path.cwd() / "runs" / run_id
			candidate = (base_dir / candidate).resolve()
	elif config.output_dir is not None:
		candidate = Path(config.output_dir) / "epoch_times.csv"
	else:
...
def apply_preprocess_config(config: dict, *, target_fs: float) -> Iterator[None]:
	...
	try:
		...
		if notch:
			...
			if base > 0.0:
				PREPROCESS["notch"] = base
		if requested_harmonics is not None:
			PREPROCESS["n_harmonics"] = int(requested_harmonics)
			PREPROCESS["notch_harmonics"] = requested_harmonics > 0
		PREPROCESS["resample"] = float(target_fs)
		yield
	finally:
		BANDPASS, NOTCH, NORMALIZE = original_flags
		PREPROCESS["n_harmonics"] = original_harmonics
		PREPROCESS["notch_harmonics"] = original_harmonics > 0
